...
void array_list_add(array_list_t *list, void *data, size_t index)
{
    ...
    if (list->data == NULL)
    {
        list->data = linear_allocator_alloc(list->allocator, list->capacity * list->element_size);
    ...
    memcpy((char *)list->data + index * list->element_size, data, list->element_size);
    list->count++;
}
...
void array_list_delete(array_list_t *list, size_t index)
{
    ...
    memmove((char *)list->data + index * list->element_size, (char *)list->data + (index + 1) * list->element_size,
